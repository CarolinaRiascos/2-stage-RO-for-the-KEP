...
//       for (int i = 0; i < auxsol.getSize(); i++){
//           auxsol[i] = IloNumArray(env);
//           cplexmPICEF.getValues(auxsol[i],aux[i]);
//           for (int j = 0; j < auxsol[i].getSize(); j++){
//               if (auxsol[i][j] > 0){
//                   cout << aux[i][j].getName() << ": " << auxsol[i][j] << endl;
//               }
//           }
//       }
